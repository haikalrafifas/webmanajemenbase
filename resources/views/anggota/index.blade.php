    ...
    <div class="col-md-4">
      <div class="card h-100">
        <div class="card-body">
          ...
          <h4>{{ $user->name }} ⭐</h4>
          <p class="mb-1">{{ $user->identitas ?? 'Mahasiswa / Anggota Base' }}</p>
          <p class="mb-0">Prodi: {{ $user->program_studi }}</p>
        </div>
      </div>
    </div>

    {{-- INI BUKAN ROW BARU, HARUSNYA LANJUT DARI ATAS --}}
    <div class="col-md-4">
...
<tbody>
 ...
 <!-- Table Tambahan: Mata Kuliah dan Deadline -->
<div class="mt-5">
  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody>
      @forelse ($mataKuliah as $index => $task)
      <tr>
        <td class="text-center">{{ $index + 1 }}</td>
      ...
      @empty
      <tr>
        <td colspan="3" class="text-center text-muted">Belum ada tugas yang tercatat.</td>
      </tr>
      @endforelse
    </tbody>
  </table>
